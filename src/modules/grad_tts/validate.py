...
def validate(args, config):
    ...
    model = GradTTSModule.load_from_checkpoint(args.ckpt_path, params=config)
    ...
    for i, p in tqdm(enumerate(data_list), total=len(data_list)):
        ...
        *inputs, _ = tokenizer(inputs)
        ...
        with torch.no_grad():
            o = model([*inputs, length], n_time_steps=TIME_STEP, temperature=TEMPERATURE, use_solver=USE_SOLVER)
            w = vocoder(o)
