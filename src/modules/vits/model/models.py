...
from ...common.model.layers.embedding import EmbeddingLayer
from .commons import init_weights, get_padding, sequence_mask, rand_slice_segments, generate_path
from .modules import (
    Log, ElementwiseAffine, ConvFlow, Flip,
    DDSConv, LayerNorm, WN, ResBlock1, ResBlock2,
    ResidualCouplingLayer, LRELU_SLOPE
)
...
class TextEncoder(nn.Module):
    def __init__(self, params):
        ...
        self.proj = nn.Conv1d(params.encoder.hidden_channels, params.n_mel * 2, 1)

    def forward(self, labels, x_lengths):
        x = self.emb(*labels)
...
class SynthesizerTrn(nn.Module):
    def __init__(self, params):
        ...
        self.enc_p = TextEncoder(params)
        self.dec = Generator(**params.generator)
        self.enc_q = PosteriorEncoder(**params.pos_enc)
        self.flow = ResidualCouplingBlock(**params.flow)

        self.dp = StochasticDurationPredictor(**params.dp)

    def forward(self, labels, x_lengths, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
        x, m_p, logs_p, x_mask = self.enc_p(labels, x_lengths)
